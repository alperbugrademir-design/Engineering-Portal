...
function eFe(pH, E, C) {
  ...
  const e_fe2_fe3o4 = 0.980 - 4*k*pH - (3*k/2)*C;
  const e_fe2_fe2o3 = 0.728 - 3*k*pH - k*C;
  const ph_fe3_fe2o3 = 1.0 - (1.0/3)*C;
  const ph_hfeo2 = 18.3 + C;
  ...
  if (E < e_fe_fe2 && E < e_fe_fe3o4) return {p:'Fe (Metal)', t:'immunity'};
  if (pH < ph_fe3_fe2o3 && E > e_fe2_fe3) return {p:'Fe3+', t:'corrosion'};
  if (E >= e_fe3o4_fe2o3 && E >= e_fe2_fe2o3 && pH >= ph_fe3_fe2o3) return {p:'Fe2O3', t:'passivity'};
  if (E >= e_fe_fe3o4 && E >= e_fe2_fe3o4 && E < e_fe3o4_fe2o3) return {p:'Fe3O4', t:'passivity'};
  if (E >= e_fe_fe2) return {p:'Fe2+', t:'corrosion'};
  return {p:'Fe (Metal)', t:'immunity'};
}
...
function eAu(pH, E, C) {
  ...
  // DÜZELTİLDİ: 'a' ve 'c' doğrularının matematiksel kesişim noktası
  const b = -0.67 - 0.333*C; 

  if (E <= a && E <= c) return {p:'Au (Metal)', t:'immunity'};
  if (pH <= b && E > a) return {p:'Au3+', t:'corrosion'};
  if (pH > b && E > c) return {p:'Au2O3', t:'passivity'};
...
function eMn(pH, E, C) {
  ...
  const e_mn2_mno4 = 1.51 - (8*k/5)*pH;
...
class BridgeDelegate(object):
    def webview_should_start_load(self, webview, url, nav_type):
        if url.startswith('app://quicklook'):
            try:
                b64_string = webview.evaluate_javascript("document.getElementById('pngPreview').src")
                if b64_string and ',' in b64_string:
                    header, data = b64_string.split(',', 1)
                    img_data = base64.b64decode(data)
                    dosya_adi = "Pourbaix_Diyagrami.png"
                    with open(dosya_adi, 'wb') as f:
                        f.write(img_data)
                    console.quicklook(dosya_adi)
            except Exception as e:
                console.alert("Hata", str(e), "Tamam", hide_cancel_button=True)
            return False
        return True

if __name__ == "__main__":
    tarayici = ui.WebView()
    tarayici.delegate = BridgeDelegate()
    tarayici.background_color = '#08090d'
